...
4. [Core Endpoints](#core-endpoints)
   ...
   - [📄 Report Generation (NEW)](#report-generation)
...
- "Show me report types"

```bash
curl "http://localhost:8001/api/v1/reports/types"
```
...
```json
{
  ...
  "timestamp": "2025-12-03T20:55:00.023273"
}
```
...
- `report_type` (required): Report type identifier (from `/types` endpoint)
- `year` (required): Report year (e.g., 2025)
- `month` (required): Report month (1-12)
- `factory_id` (optional): Factory UUID (defaults to demo factory)
...
6. **Recommendations**
   - Auto-generated efficiency suggestions

---

### EP33: GET /api/v1/reports/preview - Preview Report Data
**Purpose:** Get report data in JSON format before generating PDF

**Endpoint:** `GET /api/v1/reports/preview`
...
- "Preview this month's energy data"
- "Show me report summary before generating"

```bash
# Preview December 2025 report
curl -G "http://localhost:8001/api/v1/reports/preview" \
  --data-urlencode "report_type=monthly_enpi" \
...
```json
{
  ...
  "data": {
    ...
    "anomalies": [
      {
        "anomaly_id": "20afa6e5-05f1-4149-899d-b084c982498c",
        "detected_at": "2025-12-01T06:00:00+00:00",
        "severity": "normal",
        "is_resolved": false,
        "machine": "HVAC-Main"
      }
    ...
    "recommendations": [
      "Boiler-1 accounts for 42.0% of energy - prioritize for efficiency review"
    ]
  },
  "timestamp": "2025-12-03T20:55:08.652227"
}
```
...
- `data`: Complete report data structure
  - `year`, `month`, `period`: Report period information
  - `factory_name`: Factory identifier
  - `summary`: Overall metrics (total kWh, month-over-month change, anomaly counts)
  - `enpis`: Energy performance indicators with current/target/status
  - `machines`: Top consumers with machine_id, name, kWh, percentage, trend
  - `daily_data`: Daily consumption data (array of {date, kwh})
  - `daily_stats`: Min/max/average statistics
  - `anomalies`: Issues detected with full details (anomaly_id, detected_at, severity, machine)
  - `recommendations`: Auto-generated suggestions

**Notes:**
- ✅ Use for voice summaries without PDF generation
- ✅ Faster than PDF generation (200-500ms)
- ✅ Same data as PDF report
- ✅ JSON format for easy parsing in OVOS

**OVOS Voice Response Example:**
```
"The December 2025 energy report shows 5,263 kilowatt-hours consumed. 
Boiler-1 accounts for 42% of total energy usage. 
There is 1 normal anomaly detected. 
Would you like me to generate the full PDF report?"
```
...
- `POST /api/v1/reports/generate` - Generate and download PDF report
- `GET /api/v1/reports/preview` - Preview report data in JSON format
